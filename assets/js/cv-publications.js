(() => {
	...
	*/
		...
		*/
		...
		if (q) {
			shown.sort((a, b) => {
				...
				const titleA = String(a.record.title || '');
				const titleB = String(b.record.title || '');
	...
	function itemHTML(r) {
		const title = r.url
			? `<a href="${esc(r.url)}">${richText(r.display_title || r.title)}</a>`
			: richText(r.display_title || r.title);
		const meta = [labels[r.type] || r.type, r.source==='CSA Research' ? r.authors : null, r.date ? prettyDate(r.date) : null].filter(Boolean).join(' · ');
		const citation = r.citation ? `<p class="cv_citation">${richText(r.citation)}</p>` : '';
	...
	fetch('/data/publications.json?v=2').then(r => r.json()).then(data => { records=data.records; render(); });
